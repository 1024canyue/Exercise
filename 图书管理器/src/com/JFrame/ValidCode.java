...
public class ValidCode extends JComponent implements MouseListener {
	...
	*/
	...
	public String getCode() {      //返回自己产生的验证码
		return this.code.toUpperCase();       //统一大写：不区分大小写
	}
 
		//产生随机的颜色
	public Color getRandColor(int min, int max) {
 
		if (min > 255)min = 255;     //防止输入参数超出范围
		if (max > 255)max = 255;
		int red = random.nextInt(max - min) + min;   //随机生成RGB值
		int green = random.nextInt(max - min) + min;
		int blue = random.nextInt(max - min) + min;
		return new Color(red, green, blue);   //返回
	}
	/*
		设置验证码具体的字母是什么
	*/
	protected String generateCode() {
		...
		for (int i = 0, len = codes.length; i < len; i++) {
			if (random.nextBoolean()) {
				codes[i] = (char) (random.nextInt(26) + 65);   //acsii码
			} else {
